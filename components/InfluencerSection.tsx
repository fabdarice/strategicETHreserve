...
export default function InfluencerSection({ influencers }: { influencers: Influencer[] }) {
  return (
    <section className="space-y-6">
      <div>
        <Image src="/images/ethchampions.svg" alt="Ethereum Champions" width={250} height={50} className="mb-2" />
        <p className="text-sm">
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {influencers.map((influencer) => (
          <Card key={influencer.id} className="border-[hsl(var(--primary))] bg-card/80 backdrop-blur-sm neon-border overflow-hidden">
            <CardHeader className="flex flex-row items-center gap-4 p-6">
              ...
              <div className="flex-1">
                <h3 className="font-semibold text-[hsl(var(--primary))]">{influencer.name}</h3>
                {influencer.twitter && (
                  <a
